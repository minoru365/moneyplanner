...
test("hasHistorySearchCriteria is true when any searchable condition is set", () => {
  ...
  assert.equal(
    hasHistorySearchCriteria({ type: "all", categoryName: "食費" }),
    true,
  );
  assert.equal(
    hasHistorySearchCriteria({ type: "all", storeName: "スーパー" }),
    true,
  );
  assert.equal(
    hasHistorySearchCriteria({ type: "all", memoQuery: "弁当" }),
    true,
  );
  assert.equal(
    hasHistorySearchCriteria({ type: "all", fromDate: "2026-05-01" }),
    true,
  );
  assert.equal(
    hasHistorySearchCriteria({ type: "all", toDate: "2026-05-31" }),
    true,
  );
});
