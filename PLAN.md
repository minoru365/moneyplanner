...
- [x] 実装済み: 設定の内訳管理でもカテゴリ配下の表示順を変更できるようにし、記録/編集時の内訳選択モーダルへ反映する（issue #6、TestFlight build 30で実機確認予定）
- [x] 実装済み: 集計・履歴・金額入力・口座残高などの数値表示へ等幅数字を適用し、桁を比較しやすくする（issue #5、TestFlight build 30で実機確認予定）
