...
このファイルは現在の進捗管理と未完了タスクの一覧を主目的とする。確定済みの構成・データモデルは [ARCHITECTURE.md](ARCHITECTURE.md)、全体のドキュメント構成は [開発者向けドキュメント](docs/development.md) を参照する。

作業完了時は、変更内容に応じて [開発者向けドキュメント](docs/development.md) と、関連する設計・運用ドキュメントの更新要否を確認する。
