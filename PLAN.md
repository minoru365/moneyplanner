...
> 現在はSQLite置換実装とEAS production buildの作成実績は完了済み。build 20で見つかった課題と追加改善は2026-05-10時点で実機確認完了。次の検証対象はTestFlight build 21発行後の再確認。

#### 直近の完了と次アクション（2026-05-10）

- [x] `docs/known-issues.md` のA〜H項目をすべて対応完了
- [x] 上記対応をdev-client実機で確認し、重大な操作不具合なし
- [ ] build 21 を EAS production build で作成する
- [ ] build 21 を TestFlight へ submit し、最終スモーク確認を行う
- [ ] `docs/release-checklist.md` の未完了項目（全データ削除導線、App Check、申請前判断）を完了する
...
- [x] 6文字招待コードは維持しつつ、参加承認制・有効期限・試行制限で総当たりリスクを抑える設計にする
...
- [ ] Ticket 22: プライバシー対策の本番化（depends on Ticket 7, 17, 18）
  ...
  - [ ] App Check本体は導入済み。Expo config plugin / ネイティブ初期化の再導入を再トライし、起動前クラッシュが再発しないか TestFlight で確認する
