...
- [x] 実装済み: カテゴリリセット後も、同名支出カテゴリの予算を引き継ぎ、計画タブのプロフィール/ライフイベントは削除しない（TestFlight build 21で実機確認予定）
- [x] 実装済み: 計画タブの年間収入/年間支出は履歴合計から自動補完せず、手入力値だけをシミュレーションに使う（TestFlight build 21で実機確認予定）
...
- [x] 実装済み: 計画タブの `planProfile` / `planLifeEvents` を `onSnapshot` 購読にし、家族端末間で即時同期する（TestFlight build 21で実機確認予定）
