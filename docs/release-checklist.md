...
- [ ] CSVインポートIAPのApp Store Connect側設定と実機確認を行う（価格は初期リリース ¥300 で決定済み。アプリ側の購入/復元導線はexpo-iapで実装済み〈2026-07-03、`lib/csvImportIap.ts`〉。残タスク: ① App Store Connectで非消耗型プロダクト `csv_import_unlock` を作成 ② Family Sharing対応可否を決定・設定 ③ `EXPO_PUBLIC_CSV_IMPORT_IAP_ENABLED=1` のビルドでSandbox/TestFlightの購入・復元・キャンセルを確認（**Sandbox側は2026-07-03 dev-clientで全項目確認済み**: 購入→インポート解放、購入を復元→解放、購入シートのキャンセル、未購入アカウントでの復元→見つからない表示。残りはTestFlightビルド〈env焼き込み〉での最終確認のみ。購入直後にdev-clientが一時的に真っ白になる事象が1回発生したが、アプリ再起動で復旧・再現なし・購入処理は正常完走）。`docs/app-store-submission-draft.md`, `docs/privacy-and-monetization.md`）
