...
- [ ] CSVインポートIAPのApp Store Connect側設定と実機確認を行う（価格は初期リリース ¥300 で決定済み。アプリ側の購入/復元導線はexpo-iapで実装済み〈2026-07-03、`lib/csvImportIap.ts`〉。残タスク: ① App Store Connectで非消耗型プロダクト `csv_import_unlock` を作成 ② Family Sharing対応可否を決定・設定 ③ `EXPO_PUBLIC_CSV_IMPORT_IAP_ENABLED=1` のビルドでSandbox/TestFlightの購入・復元・キャンセルを確認。`docs/app-store-submission-draft.md`, `docs/privacy-and-monetization.md`）
