...
- [ ] App Storeプライバシー栄養表示を確認する（build 31のarchive inspectionでGoogle Analytics SDKの存在を確認。2026-07-11、Analytics収集を無効化する方向で決定し `app.json` に `FIREBASE_ANALYTICS_COLLECTION_DEACTIVATED: true` を設定済み。次回buildでのproduction archive再確認とTestFlight再検証、およびApp Store Connect最終入力の人間レビューが未完了。詳細は `docs/app-privacy-audit.md`、`docs/app-store-submission-draft.md`）
