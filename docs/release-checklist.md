...
- [ ] App Store審査却下（Guideline 5.1.1(v)、2026-07-15受領）への対応と再提出
  - [x] アカウント削除導線の改善を実装（世帯設定画面に削除導線追加、設定画面の文言を「アカウントを削除（全データ削除）」へ変更しアカウントセクションを既定展開、Apple Sign-Inトークン失効を追加。詳細は `docs/decisions/account-deletion-store-compliance.md`）
  - [ ] TestFlight/dev-client実機で、世帯未参加時・世帯参加時それぞれのアカウント削除フローを確認する
  - [ ] 実機で「サインイン → 削除導線へ移動 → 削除完了」の画面録画を撮影する
  - [ ] 新ビルド（build 35）を作成・提出し、App Reviewへの返信に画面録画を添付する（App Review Informationのメモ欄にも録画を残す）
