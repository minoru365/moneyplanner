...
- 告知する内容は、発生時刻、影響範囲、現在の状態、ユーザーに必要な操作、復旧見込み、次回更新予定に絞る
- 家計明細、個人情報、認証情報、内部ID、Firebase設定値は告知に含めない
- データ復旧できない可能性がある場合は、推測で断定せず、確認済み事実、復旧を試した範囲、ユーザー側で取り得る対応（CSV出力済みファイルの保管など）を明記する
- 誤削除や同期不整合では、対象範囲の特定、バックアップ/エクスポート有無、復旧可否判断、再発防止策を記録する
- 有料機能（CSVインポートIAP）に影響する障害では、購入状態、復元購入、返金申請先（Apple）を案内する。ただし返金可否はAppleの判断であることを明記する
