...
> Firestore原価とApple手数料控除後の価格シナリオは [docs/firestore-read-and-index-plan.md#具体的な月次原価試算料金決定の根拠](firestore-read-and-index-plan.md) を参照。1世帯あたり実勢¥0.1〜¥10/月、スケール時はサブスク¥240/月または年額¥2,400で粗利確保可能と試算済み。
...
- [ ] Apple手数料控除後の手取りを前提に、最低維持価格・目標利益価格・キャンペーン価格を試算する（試算結果あり、最終価格は競合調査後に確定）
