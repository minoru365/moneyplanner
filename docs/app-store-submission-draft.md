...
- 未購入時も記録追加、同期、閲覧、CSV出力、アカウント削除（全データ削除）は制限しない
...
- **データ削除について**: アカウント削除（全データ削除）、世帯削除、メンバー解除は購入状態に関係なく利用できます。
...
## 7. 商品ページ文言（名称・サブタイトル・プロモーション用テキスト・説明文・キーワード）ドラフト

`site/index.html` と本ドキュメント既存節（3〜5）で確認済みの表現のみを使い、新しい効能・実績を主張しない方針で作成した文字数目安付きの下書き。最終的な文字数はApp Store Connect入力画面の表示で確認する。
...
目安15文字（上限30文字）。既存のプライバシーポリシー・サイトタイトルと一致。
...
目安15文字（上限30文字）。`site/index.html` の特徴セクション見出しと同一表現にして、サイトとストアの訴求を揃えている。

### プロモーション用テキスト（Promotional Text、審査不要で随時更新可）

```text
家族やパートナーと、日々の家計を気持ちよく共有するiPhone家計簿アプリ。元々自分たちが使う理想の家計簿アプリが欲しくて開発しました。CSV出力は無料。CSVインポートは¥300の買い切りIAP（サブスクなし）で解放できます。
```

114文字（上限170文字）。2026-07-12にApp Store Connectへ入力した文面（ユーザーが開発動機の一文を追加した版）。

### 説明文（Description）

```text
NANBOは、家族やパートナーと日々の家計を気持ちよく共有するためのiPhone家計簿アプリです。

■ 世帯で共有
世帯のメンバーと家計簿を共有し、それぞれの記録をリアルタイムに確認できます。

■ 安心してサインイン
Apple Sign-Inでかんたんに始められます。

■ リアルタイム同期
Cloud Firestoreにより、世帯内の記録・カテゴリ・予算の変更がリアルタイムに同期されます。

■ 記録・履歴・集計
日々の収支をすばやく記録し、履歴は検索・絞り込みで見つけやすく、集計はカテゴリ別・月別に確認できます。カテゴリごとに予算を設定できます。

■ CSVの入出力
コア機能、世帯共有、CSVエクスポートはすべて無料で利用できます。
CSVインポートは非消耗型の買い切りアプリ内課金（¥300）で解放できます。サブスクリプションではなく、購入は購入したApple IDに紐づきます（Family Sharingでは共有されません）。エクスポート済みのCSVや対応形式のCSVから、取引データをまとめて取り込めます。

■ データの取り扱い
家計簿データは、開発・分析・広告・外部提供へ二次利用しません。詳しくはプライバシーポリシーをご確認ください。

■ お問い合わせ
不具合の報告やご質問は、GitHub Issuesからお寄せください。
```

上限4000文字に対し余裕あり。「■」見出しは装飾文字のため、App Store Connect側の表示崩れがないか入力後に確認する。

### キーワード（Keywords、カンマ区切り・空白なし、上限100文字）

```text
家計簿,家計,共有,世帯,夫婦,カップル,家族,支出管理,収支,予算,CSV,同期
```

目安44文字。ASOのキーワード調査データに基づくものではなく、機能・利用シーンからの素案。競合アプリ調査後に見直しの余地あり。

注: 上記はすべて下書き。App Store Connect入力前に、表示名・サブタイトル・説明文・キーワードが実装とプライバシーポリシーの記述（データの二次利用なし、IAP¥300・Family Sharing無効など）と矛盾しないか最終確認する。
