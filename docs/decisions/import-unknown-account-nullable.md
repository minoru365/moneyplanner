...
> **2026-06-14 追記（この節は後続決定で更新）**: その後、CSVインポートで約1万件を投入した結果、
> 口座ピッカー/口座管理を開くたびに走る自動 reconcile が「重い」かつ「インポート分を残高へ折り込む」
> 問題が判明した。これを受け **自動 reconcile を廃止**し、残高は手動＋増分のみで維持する方針に変更した。
> 詳細は [account-balance-incremental-only.md](account-balance-incremental-only.md) を参照。
